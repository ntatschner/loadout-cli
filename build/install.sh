#!/bin/sh
# Installs agentctl on Linux or macOS from a release archive.
#
...
if [ -z "$ARCHIVE" ]; then
    ...
    ARCHIVE=$(ls -t "${script_directory}"/agentctl-*.tar.gz 2>/dev/null | head -1 || true)
fi
...
if [ ! -f "${temporary}/agentctl" ]; then
    echo "The archive does not contain agentctl." >&2
    exit 1
fi
...
install -m 0755 "${temporary}/agentctl" "${BIN_DIR}/agentctl"

echo "Installed ${BIN_DIR}/agentctl"
...
if [ "$(uname -s)" = "Darwin" ] && command -v xattr >/dev/null 2>&1; then
    if xattr -p com.apple.quarantine "${BIN_DIR}/agentctl" >/dev/null 2>&1; then
        xattr -d com.apple.quarantine "${BIN_DIR}/agentctl" 2>/dev/null || true
