...
/// <summary>The interactive launcher shown when agentctl is run with no arguments.</summary>
...
public sealed class LauncherTui : ILauncherTui
{
    ...
    public async Task<int> RunAsync(CancellationToken ct = default)
    {
        ...
        if (projects.Count == 0)
        {
            ...
            _console.MarkupLine("[dim]Register one with:[/] agentctl project add <path>");
            _console.MarkupLine("[dim]Or find existing repositories with:[/] agentctl project discover");
        ...
        CancellationToken ct)
    {
        ...
        if (project.LocalPath is null)
        {
            ...
            _console.MarkupLine(
                $"[dim]Clone it with:[/] agentctl project clone {Markup.Escape(project.Entry.Slug)}");
