...
public sealed class MigrateCommand : AsyncCommand<MigrateCommand.Settings>
{
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings)
    {
        ...
        if (applied.BackupId is not null)
        {
            ...
            output.WriteLine(
                $"[dim]Undo it with:[/] loadout backup restore {Markup.Escape(applied.BackupId)}");
        }
    ...
    private void WriteInstructionHint(CommandOutput output, MigrationPlan plan)
    {
        ...
        output.WriteLine($"  loadout rules budget {Markup.Escape(plan.Slug)}");
        output.WriteLine($"  loadout rules split {Markup.Escape(plan.Slug)} --write-map");
    }
