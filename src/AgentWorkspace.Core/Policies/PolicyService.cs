...
public sealed class PolicyService : IPolicyService
{
    ...
    private const string ExcludeFileName = "agentctl-global-excludes";
    ...
    /// </summary>
    private const string HookSignature = "# agentctl-managed-hook";
    ...
    public async Task<OperationResult<string>> InstallGlobalExcludesAsync(CancellationToken ct = default)
    {
        ...
        var builder = new StringBuilder()
            .AppendLine("# Written by agentctl. Agent tooling files are kept out of application")
    ...
    /// agentctl, so the protection still works on a machine where the launcher
