...
public interface IGitManager
{
    ...
    /// <summary>
    /// Removes a key from one repository's own configuration.
    /// <para>
    /// Used to clear a mark left by a previous name of this tool. Leaving both
    /// in place would mean two answers to the same question, free to drift
    /// apart the moment a project is renamed.
    /// </para>
    /// </summary>
    Task<OperationResult> RemoveLocalConfigValueAsync(
        string key,
        string repositoryPath,
        CancellationToken ct = default);
