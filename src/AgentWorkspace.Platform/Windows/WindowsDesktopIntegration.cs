...
public sealed class WindowsDesktopIntegration : IDesktopIntegration
{
    ...
    public async Task<OperationResult> InstallAsync(string executablePath, CancellationToken ct = default)
    {
        ...
        {
            ["AGENTCTL_SHORTCUT_PATH"] = shortcutPath,
            ["AGENTCTL_TARGET_PATH"] = executablePath,
        };

        const string Script =
            "$shell = New-Object -ComObject WScript.Shell; "
            + "$link = $shell.CreateShortcut($env:AGENTCTL_SHORTCUT_PATH); "
            + "$link.TargetPath = $env:AGENTCTL_TARGET_PATH; "
