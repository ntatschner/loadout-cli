    ...
    public OperationResult<string> GetCompletionInstallPath(ShellKind shell)
    {
        ...
        return shell switch
        {
            ...
            ShellKind.Zsh => OperationResult<string>.Ok(
                Path.Combine(home, ".zfunc", "_agentctl")),

            ShellKind.Bash => OperationResult<string>.Ok(
                Path.Combine(ResolveXdgData(home), "bash-completion", "completions", "agentctl")),

            ShellKind.Fish => OperationResult<string>.Ok(
                Path.Combine(ResolveXdgConfig(home), "fish", "completions", "agentctl.fish")),

            ShellKind.PowerShell => OperationResult<string>.Ok(
                Path.Combine(home, ".config", "powershell", "agentctl.completion.ps1")),
