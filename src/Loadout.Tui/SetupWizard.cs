...
public sealed class SetupWizard : ISetupWizard
{
    ...
    public async Task<int> RunAsync(SetupRequest request, CancellationToken ct = default)
    {
        ...
        _console.MarkupLine("[dim]Run[/] loadout [dim]to open the launcher, or[/] loadout doctor "
        ...
        CancellationToken ct)
    {
        ...
        if (cloneResult.Failed)
        {
            return OperationResult.Fail(
                ...
                + "Check the URL and your Git credentials, then run: loadout setup");
        }
            ...
            && manifest.Value!.WorkspaceSchema > WorkspaceManager.SupportedSchemaVersion)
        {
            return OperationResult.Fail(
                ...
                + $"this launcher supports ({WorkspaceManager.SupportedSchemaVersion}). Update loadout.");
        }
        ...
        CancellationToken ct)
    {
        ...
        if (host == WorkspaceHost.None)
        {
            _console.MarkupLine(
                ...
                + "loadout config set workspace-remote <url>");
    ...
    private async Task ConfigureDiscoveryRootsAsync(SetupRequest request, CancellationToken ct)
    {
        ...
        else
        {
            ...
            if (!request.Interactive)
            {
                ...
                _console.MarkupLine(
                    "[dim]Set one later with:[/] loadout config set discovery-roots <path>");
    ...
    private async Task ShowDetectedAgentsAsync(CancellationToken ct)
    {
        ...
        {
            _console.MarkupLine(
                "[yellow]No agents were found. Install Claude Code or Codex, then run:[/] loadout doctor");
        }
    }
