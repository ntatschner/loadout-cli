...
public sealed class LinuxPaths : PlatformPathsBase
{
    ...
    private const string LegacyAppFolder = "agent-workspace-launcher";
    ...
    {
        var config = Adopt(ResolveXdg("XDG_CONFIG_HOME", ".config"), AppFolder, LegacyAppFolder);
        ...
        var data = Adopt(
            ResolveXdg("XDG_DATA_HOME", Path.Combine(".local", "share")), AppFolder, LegacyAppFolder);

        var state = Adopt(
            ResolveXdg("XDG_STATE_HOME", Path.Combine(".local", "state")), AppFolder, LegacyAppFolder);

        var cache = Adopt(ResolveXdg("XDG_CACHE_HOME", ".cache"), AppFolder, LegacyAppFolder);
