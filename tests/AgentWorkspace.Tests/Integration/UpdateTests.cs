...
public sealed class UpdateTests : IDisposable
{
    ...
    public UpdateTests()
    {
        _root = Path.Combine(Path.GetTempPath(), "agentctl-update-" + Guid.NewGuid().ToString("N"));
        ...
        _installedExecutable = Path.Combine(installDirectory, "agentctl");
    ...
    public async Task An_unconfigured_source_says_so_rather_than_failing_obscurely()
    {
        ...
        result.Error.Should().Contain("agentctl config set updates-source");
    }
    ...
    private async Task PublishAsync(
        ...
        string executableName = "agentctl",
        ...
        long? declaredSize = null)
    {
        ...
        var archivePath = Path.Combine(releases, $"agentctl-{version}.zip");
