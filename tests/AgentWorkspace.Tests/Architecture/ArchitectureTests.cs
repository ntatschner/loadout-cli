...
namespace AgentWorkspace.Tests.Contracts;
...
public sealed class ArchitectureTests
{
    ...
    [
        "AgentWorkspace.Platform.Windows",
        "AgentWorkspace.Platform.Linux",
        "AgentWorkspace.Platform.MacOS",
        "AgentWorkspace.Platform.Unix",
    ];

    /// <summary>
    /// Assemblies that must stay platform-neutral. The selector in
    /// AgentWorkspace.Platform and the composition root in the CLI are the only
    /// places allowed to name a concrete platform.
    /// </summary>
    public static TheoryData<string> NeutralAssemblies =>
    [
        PathOf<AgentWorkspace.Models.ExitCode>(),
        PathOf(typeof(AgentWorkspace.Core.ServiceRegistration)),
        PathOf<AgentWorkspace.Agents.IAgentAdapter>(),
        PathOf<AgentWorkspace.Tui.LauncherTui>(),
    ];

    public static TheoryData<string> AllProductAssemblies =>
    [
        PathOf<AgentWorkspace.Models.ExitCode>(),
        PathOf(typeof(AgentWorkspace.Platform.PlatformServices)),
        PathOf(typeof(AgentWorkspace.Core.ServiceRegistration)),
        PathOf<AgentWorkspace.Agents.IAgentAdapter>(),
        PathOf<AgentWorkspace.Tui.LauncherTui>(),
        PathOf(typeof(AgentWorkspace.Cli.Program)),
    ];
    ...
    public void Shared_assemblies_never_reference_a_platform_implementation(string assemblyPath)
    {
        ...
        violations.Should().BeEmpty(
            "shared code must depend on AgentWorkspace.Platform.Abstractions only, so that adding a "
