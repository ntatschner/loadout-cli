...
```bash
curl -fsSL .../install.sh | sh -s -- --version 0.52.0     # a particular release
...
sh install.sh --archive loadout-0.52.0-linux-x64.tar.gz   # an archive you already have
```

```powershell
./install.ps1 -Version 0.52.0
...
```powershell
msiexec /i loadout-0.52.0-win-x64.msi        # per-user, no elevation
```

```bash
sudo dpkg -i loadout_0.52.0_amd64.deb        # or: sudo rpm -i loadout-0.52.0-1.x86_64.rpm
```
...
```bash
pwsh ./build/package.ps1 -Runtime linux-x64 -Version 0.52.0     # archive
pwsh ./build/installer.ps1 -Runtime win-x64 -Version 0.52.0     # native installer
```
...
```bash
pwsh ./build/package.ps1 -Runtime osx-arm64 -Version 0.52.0
```
...
```json
{
  ...
  "artifacts": {
    "osx-arm64": {
      "url": "https://internal.example/loadout/loadout-0.52.0-osx-arm64.tar.gz",
      "sha256": "985daa42...",
      "size": 31110221
    }
  }
