...
```bash
sudo dpkg -i loadout_0.37.1_amd64.deb        # or: sudo rpm -i loadout-0.37.1-1.x86_64.rpm
```
...
```bash
pwsh ./build/package.ps1 -Runtime linux-x64 -Version 0.37.1     # archive
pwsh ./build/installer.ps1 -Runtime win-x64 -Version 0.37.1     # native installer
```
...
```bash
pwsh ./build/package.ps1 -Runtime osx-arm64 -Version 0.37.1
```
...
```json
{
  ...
  "artifacts": {
    "osx-arm64": {
      "url": "https://internal.example/loadout/loadout-0.37.1-osx-arm64.tar.gz",
      "sha256": "985daa42...",
      "size": 31110221
    }
  }
