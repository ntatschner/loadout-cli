...
```bash
tar -xzf loadout-0.31.0-linux-x64.tar.gz
...
```powershell
msiexec /i loadout-0.31.0-win-x64.msi        # per-user, no elevation
```

```bash
sudo dpkg -i loadout_0.31.0_amd64.deb        # or: sudo rpm -i loadout-0.31.0-1.x86_64.rpm
```
...
```bash
pwsh ./build/package.ps1 -Runtime linux-x64 -Version 0.31.0     # archive
pwsh ./build/installer.ps1 -Runtime win-x64 -Version 0.31.0     # native installer
```
...
```bash
pwsh ./build/package.ps1 -Runtime osx-arm64 -Version 0.31.0
```
...
```json
{
  ...
  "artifacts": {
    "osx-arm64": {
      "url": "https://internal.example/loadout/loadout-0.31.0-osx-arm64.tar.gz",
      "sha256": "985daa42...",
      "size": 31110221
    }
  }
