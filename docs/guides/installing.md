...
- Windows: `loadout-0.42.1-win-x64.msi`, or the `.zip` if you'd rather manage
  `PATH` yourself. On an ARM machine, take the `win-arm64` build.
- Linux: `loadout-0.42.1-linux-x64.tar.gz`, or the `.deb` or `.rpm`. On an ARM
...
```sh
sudo dpkg -i loadout_0.42.1_amd64.deb
```
