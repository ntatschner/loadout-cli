...
- Windows: `loadout-0.48.1-win-x64.msi`, or the `.zip` if you'd rather manage
  `PATH` yourself. On an ARM machine, take the `win-arm64` build.
- Linux: `loadout-0.48.1-linux-x64.tar.gz`, or the `.deb` or `.rpm`. On an ARM
...
```powershell
msiexec /i loadout-0.48.1-win-x64.msi
```
...
```sh
sudo dpkg -i loadout_0.48.1_amd64.deb
```
