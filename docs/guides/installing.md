...
- Windows: `loadout-0.44.0-win-x64.msi`, or the `.zip` if you'd rather manage
  `PATH` yourself. On an ARM machine, take the `win-arm64` build.
- Linux: `loadout-0.44.0-linux-x64.tar.gz`, or the `.deb` or `.rpm`. On an ARM
...
```powershell
msiexec /i loadout-0.44.0-win-x64.msi
```
...
```sh
sudo dpkg -i loadout_0.44.0_amd64.deb
```
